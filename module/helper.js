...
export class EntitySheetHelper {
        ...
        for (let i = 0; i < groups.length; i++) {
          if (groups[i].value === val) {
            ui.notifications.error(
              game.i18n.localize("MarchingOrder.NotifyAttrDuplicate") +
                ` (${val})`
            );
            el.value = oldVal;
            attrError = true;
  ...
  static validateGroup(groupName, document) {
    ...
    if (groups.includes(groupName)) {
      ui.notifications.error(
        game.i18n.localize("MarchingOrder.NotifyGroupDuplicate") +
          ` (${groupName})`
      );
      return false;
    }
    ...
    if (attributes.includes(groupName)) {
      ui.notifications.error(
        game.i18n.localize("MarchingOrder.NotifyGroupAttrDuplicate") +
          ` (${groupName})`
      );
      return false;
    }
    ...
    if (["attr", "attributes"].includes(groupName)) {
      ui.notifications.error(
        game.i18n.format("MarchingOrder.NotifyGroupReserved", {
          key: groupName,
        })
      );
      return false;
    ...
    if (groupName.match(/[\s|\.]/i)) {
      ui.notifications.error(
        game.i18n.localize("MarchingOrder.NotifyGroupAlphanumeric")
      );
      return false;
    }
    return true;
  ...
  static async deleteAttributeGroup(event, app) {
    ...
    new Dialog({
      title: game.i18n.localize("MarchingOrder.DeleteGroup"),
      content: `${game.i18n.localize(
        "MarchingOrder.DeleteGroupContent"
  ...
  static async createDialog(data = {}, options = {}) {
    ...
    const templates = collection.filter((a) =>
      a.getFlag("foundryvtt-marching-order", "isTemplate")
    );
    const defaultType = this.metadata.types[0];
    const types = {
      [defaultType]: game.i18n.localize("MarchingOrder.NoTemplate"),
    };
    ...
    const useEntity = game.marchingOrder.useEntity;
    const template = `templates/sidebar/${
      useEntity ? "entity" : "document"
    }-create.html`;
    ...
    return Dialog.prompt({
      ...
      callback: (html) => {
        ...
        if (template) {
          ...
          delete createData.flags.marchingOrder.isTemplate;
        }
  ...
  static clampResourceValues(attrs) {
    ...
    for (const [attr, value] of Object.entries(flat)) {
      ...
      foundry.utils.setProperty(
        attrs,
        attr,
        Math.clamped(value, current.min || 0, current.max || 0)
      );
    }
  }
}
